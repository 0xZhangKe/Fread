...
interface BlueskyComponent : BlueskyPlatformComponent {
    ...
    @IntoMap
    @Provides
    fun provideUserListViewModel(creator: (PlatformLocator, UserListType, String?, String?) -> UserListViewModel): Pair<ViewModelKey, ViewModelFactory> {
        return UserListViewModel::class to UserListViewModel.Factory { role, type, postUri, did ->
            creator(role, type, postUri, did)
        }
    }

    @IntoMap
    @Provides
    fun providePublishPostViewModel(creator: (PlatformLocator, String?, String?, String?) -> PublishPostViewModel): Pair<ViewModelKey, ViewModelFactory> {
        return PublishPostViewModel::class to PublishPostViewModel.Factory { role, defaultText, reply, quote ->
            creator(role, defaultText, reply, quote)
        }
    }
