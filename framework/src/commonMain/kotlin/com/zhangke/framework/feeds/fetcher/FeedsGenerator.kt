...
class FeedsGenerator<Value> {
    ...
    fun generate(
        ...
        val maxLastDatetime = paramsList
            .filter { it.statusList.isNotEmpty() }
            .minOf { params -> params.pagingSource.getDatetime(params.statusList.last()) }
        ...
        paramsList.forEach { param ->
            ...
            val lastOne = statusList.lastOrNull {
                pagingSource.getDatetime(it) >= maxLastDatetime
            } ?: return@forEach
        ...
        return GenerateResult(
            list = resultList.sortedByDescending { it.first.getDatetime(it.second) }
                .map { it.second },
